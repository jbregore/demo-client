...
export default function ZReadReceipt({ open, setOpen, setOpenMainModal, data }) {
  const settings = JSON.parse(localStorage.getItem('settings'));

  const handlePrintAgain = async () => {
    const apiData = {
      zReadData: data.zReadData,
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/reports/z-read/print`, formData, {
  ...
  const { version } = data.zReadData;

  // if (!version.startsWith('2')) {
  //   return (
  //     <StyledModal open={open} BackdropComponent={Backdrop}>
  //       <ModalCard sx={{ width: 500 }}>
  //         <Box component="section" p={2} pb={1}>
  //           <Typography variant="h6" pb={2}>
  //             Message
  //           </Typography>
  //           <Typography variant="body1">
  //             The data format of this Z-Read receipt is incompatible with this version of the POS.
  //             Please ask your administrator to regenerate the Z-Read logs.
  //           </Typography>
  //         </Box>
  //         <Divider sx={{ my: 2 }} />
    ...
    <StyledModal open={open} BackdropComponent={Backdrop}>
      <ModalCard>
        <Scrollbar sx={{ maxHeight: '80vh', px: 2 }}>
          <ReceiptHeader title="Z-Reading" isReading={true} />

          <Box component="section" sx={{ my: 1, pb: 1, borderBottom: '2px dashed #000' }}>
            <Typography noWrap variant="subtitle2" sx={{ whiteSpace: 'initial' }}>
              {`SHIFT CLOSING of ${data.cashier.lastname}, ${data.cashier.firstname} - ${data.cashier.id}`}
            </Typography>
            <Typography noWrap variant="subtitle2">
            ...
            <Typography noWrap variant="subtitle2">
              {`Transaction date: ${moment(
                data.zReadData.initialFund.INITIAL_FUND[0].cashDate
              ).format('MM-DD-YYYY')}`}
            </Typography>
            <Typography noWrap variant="subtitle2">
          ...
          <Box component="section" sx={{ my: 1, pb: 1, borderBottom: '2px dashed #000' }}>
            ...
            {payments.cashOnDelivery?.LALAMOVE?.count > 0 && (
              <>
                <Typography noWrap variant="subtitle2"> *** LALAMOVE *** </Typography>
                <Stack direction="row" justifyContent="space-between">
                  <Typography noWrap variant="subtitle2">
            ...
            {payments.cashOnDelivery?.LBC?.count > 0 && (
              <>
                <Typography noWrap variant="subtitle2"> *** LBC *** </Typography>
              <Stack direction="row" justifyContent="space-between">
                <Typography noWrap variant="subtitle2">
            ...
            {payments.cashOnDelivery?.PAYO?.count > 0 && (
              <>
                <Typography noWrap variant="subtitle2"> *** PAYO *** </Typography>
                <Stack direction="row" justifyContent="space-between">
                  <Typography noWrap variant="subtitle2">
            ...
            {payments.cashOnDelivery?.WSI?.count > 0 && (
              <>
                <Typography noWrap variant="subtitle2"> *** WSI *** </Typography>
                <Stack direction="row" justifyContent="space-between">
                  <Typography noWrap variant="subtitle2">
            ...
            {payments.cashOnDelivery?.CONSEGNIA?.count > 0 && (
              <>
                <Typography noWrap variant="subtitle2"> *** CONSEGNIA *** </Typography>
                <Stack direction="row" justifyContent="space-between">
                  <Typography noWrap variant="subtitle2">
            ...
            )}
            {
              Object.keys(payments.nonCash.cards)
                .filter((label) => !(['CREDIT_CARD', 'DEBIT_CARD'].includes(label)) && payments.nonCash.cards[label].count > 0)
            ...
            {payments.nonCash.giftCards.summary.count > 0 && (
              <>
                <Box>
                  ...
                  {payments.nonCash.giftCards.GC_ITEMS_METHODS.map((gift, index) => (
          ...
          <Box component="section" sx={{ my: 1, pb: 1, borderBottom: '2px dashed #000' }}>
            ...
            <Stack direction="row" justifyContent="space-between">
              <Typography noWrap variant="subtitle2">
                {`VAT (${data.zReadData.isNonVat ? 0 : vat.count})`}
              </Typography>
              <Typography noWrap variant="subtitle2">
                {data.zReadData.isNonVat ? '0.00' : fCurrency('', roundUpAmount(vat.total))}
              </Typography>
            </Stack>
          </Box>
          <Box component="section" sx={{ my: 1, pb: 1, borderBottom: '2px dashed #000' }}>
            <Stack direction="row" justifyContent="space-between">
              <Typography noWrap variant="subtitle2">
                {`TOTAL VAT (${data.zReadData.isNonVat ? 0 : vat.count})`}
              </Typography>
              <Typography noWrap variant="subtitle2">
                {data.zReadData.isNonVat ? '0.00' : fCurrency('', roundUpAmount(vat.total))}
              </Typography>
          ...
          <Box component="section" sx={{ my: 1, pb: 1, borderBottom: '2px dashed #000' }}>
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography noWrap variant="subtitle2">
                {fCurrency(
                  '',
                  roundUpAmount(data.zReadData.isNonVat ? 0 : vat.VAT_DETAILS.vatableSales)
                )}
              </Typography>
            </Stack>
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography noWrap variant="subtitle2">
                {fCurrency(
                  '',
                  roundUpAmount(data.zReadData.isNonVat ? 0 : vat.VAT_DETAILS.vatAmount)
                )}
              </Typography>
            </Stack>
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography noWrap variant="subtitle2">
                {fCurrency('', roundUpAmount(vat.VAT_DETAILS.vatExemptSales))}
              </Typography>
            </Stack>
            <Stack direction="row" justifyContent="space-between">
              <Typography noWrap variant="subtitle2">
              ...
              <Typography noWrap variant="subtitle2">
                {fCurrency(
                  '',
                  roundUpAmount(data.zReadData.isNonVat ? 0 : vat.VAT_DETAILS.vatZeroRated)
                )}
              </Typography>
            </Stack>
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography noWrap variant="subtitle2">
                {fCurrency('', roundUpAmount(vat.VAT_DETAILS.nonVatable))}
              </Typography>
            </Stack>
          </Box>
          <Box component="section" sx={{ my: 1, pb: 1, borderBottom: '2px dashed #000' }}>
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography noWrap variant="subtitle2">
                {fCurrency('', roundUpAmount(SALES.net))}
              </Typography>
            </Stack>
          </Box>
          <Box component="section" sx={{ my: 1, pb: 1, borderBottom: '2px dashed #000' }}>
          ...
          <Box component="section" sx={{ my: 1, pb: 1, borderBottom: '2px dashed #000' }}>
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography noWrap variant="subtitle2">
                {fCurrency(
                  '',
                  initialFund.INITIAL_FUND[0] ? roundUpAmount(cashDrop.TOTAL_IN_DRAWER) : '0.00'
                )}
              </Typography>
            </Stack>
            <Typography noWrap variant="subtitle2">
            ...
            {payments.nonCash.giftCards.summary.EXCESS_GC_AMOUNT.total > 0 && (
              <Stack direction="row" justifyContent="space-between">
                <Typography noWrap variant="subtitle2">
                  GIFT CARD CHANGE
                </Typography>
          ...
          <Box component="section" sx={{ my: 1, pb: 1, borderBottom: '2px dashed #000' }}>
            ...
            <Typography noWrap variant="subtitle2">
              {fCurrency('', roundUpAmount(ACCUMULATED_SALES.old))}
            </Typography>
            <Typography noWrap variant="subtitle2">
              NEW ACCUMULATED SALES:
            </Typography>
            <Typography noWrap variant="subtitle2">
              {fCurrency('', roundUpAmount(ACCUMULATED_SALES.new))}
            </Typography>
            <Typography noWrap variant="subtitle2">
              {`ZREAD COUNT: ${data.zReadData.zReadLogsCount + 1}`}
            </Typography>
          </Box>
