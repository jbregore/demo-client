...
export default function VoidedSales({ open, setOpen }) {
  ...
  const fetchVoid = async () => {
    ...
    try {
      const voidReportsData = await axios.get(
        `${process.env.REACT_APP_API_URL}/reports/voided-sales/${storeCode}?${objectToString(
          initialParams
        )}`
      );
  ...
  const handlePrintCsv = () => {
    ...
    downloadCsv(formattedData, columns, `${filename}_VOIDED-SALES`);
    ...
    addUserActivityLog(
      ...
      `${capitalCase(storedData.user.firstname)} ${capitalCase(
        storedData.user.lastname
      )} has downloaded a CSV report for voided sales.`,
      'Download CSV Voided Sales Report',
      `${
        posDateData[0]
    ...
    <StyledModal open={open} onClose={() => setOpen(false)} BackdropComponent={Backdrop}>
      <ModalCard>
        <Stack direction="row" justifyContent="space-between" px={2}>
          <Typography variant="h6">Voided Sales</Typography>
          <Button
            variant="contained"
