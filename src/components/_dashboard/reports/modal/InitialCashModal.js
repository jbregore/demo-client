...
export default function InitialCash({ open, setOpen, user, report }) {
  ...
  useEffect(() => {
    ...
    } else if (storedData?.user.role === 'cashier' && !devMode) {
      setOpenDrawer();
    }
  ...
  const handleLogInitialCash = async () => {
    setIsLoading(true);
    ...
    try {
      ...
      const result = await axios.post(`${Endpoints.REPORTS}/initial`, payload, {
        withCredentials: true
      });

      if (result.status === 200) {
        localStorage.setItem('initialCash', true);
  ...
  return (
    <StyledModal open={open} onClose={() => setOpen(false)} BackdropComponent={Backdrop}>
      <ModalCard>
        <Typography variant="h6">Initial Cash</Typography>
        <Box mt={3}>
