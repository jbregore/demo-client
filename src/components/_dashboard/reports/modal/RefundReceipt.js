...
export default function RefundReceipt({ open, setOpen, fullData }) {
  const { data } = fullData;
  ...
  const handlePrintAgain = async () => {
    ...
    try {
      await axios.post(`${Endpoints.TRANSACTION}/refund/receipt`, formData, {
    ...
    <StyledModal open={open} BackdropComponent={Backdrop}>
      <ModalCard>
        <Scrollbar sx={{ maxHeight: '80vh', px: 2 }}>
          <ReceiptHeader title="REFUND" vatable={data.cart.isNonVat} />
          ...
          <Box sx={{ my: 1, borderBottom: '2px dashed #000' }}>
            ...
            <Typography noWrap variant="subtitle2" mt={1} mb={1}>
              {`REFUND Remarks: ${data.cart.remarks}`}
            </Typography>
          </Box>
