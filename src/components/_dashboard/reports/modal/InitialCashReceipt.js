...
export default function InitialCashReceipt({ open, setOpen, closeMainModal, data }) {
  ...
  const handlePrintAgain = async () => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/reports/initial/print`, formData, {
    ...
    <StyledModal open={open} BackdropComponent={Backdrop}>
      <ModalCard>
        <Scrollbar sx={{ maxHeight: '80vh', px: 2 }}>
          <ReceiptHeader title="I N I T I A L C A S H" vatable={data.cashReport.isNonVat} />
            ...
            <Grid item xs={7}>
              <Box component="section">
                ...
                <Typography
                  noWrap
                  variant="subtitle2"
                >{`: ${data.cashReport.cashierFirstName.toUpperCase()} ${data.cashReport.cashierLastName.toUpperCase()} (${
                  data.cashReport.employeeId
                })`}</Typography>
