...
export default function RegularReceipt({ open, setOpen, data, doublePrinting }) {
  ...
  const handlePrintAgain = async () => {
    const apiData = {
      ...
      isReprint: doublePrinting ? false : true
    };
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/print-receipt`, formData, {
    ...
    if (mallAccr === 'robinson') {
      try {
        const dateOnly = moment(
          new Date(data.cart.confirmOrders[0].products[0].specsDate)
        ).format('YYYY-MM-DD');

        const reprintData = {
          type: data.cart.payments.filter((x) => x.value === 'rmes').length
            ? 'exchange'
            : 'regular',
    ...
    <StyledModal open={open} BackdropComponent={Backdrop}>
      <ModalCard>
        <Scrollbar sx={{ maxHeight: '80vh', px: 2 }}>
          <ReceiptHeader title="SALES INVOICE" vatable={data.cart.isNonVat}/>
          ...
          <Box sx={{ my: 1, borderBottom: '2px dashed #000' }}>
            ...
            {data.cart.payments.filter((x) => x.value === 'rmes').length > 0 && (
              <>
                <Typography noWrap variant="subtitle2" mt={1}>
                  {` Return Ref No. ${
                    data.cart.payments.filter((x) => x.value === 'rmes')[0].siNumber
                  }`}
                </Typography>
                <Typography noWrap variant="subtitle2">
                  {`Orig Trans Date: ${
                    fDateTimeSuffix(
                      data.cart.payments.filter((x) => x.value === 'rmes')[0].origTransactionDate
                    ).split(' ')[0]
                  }`}
                </Typography>
                <Typography noWrap variant="subtitle2">
          ...
          <ReceiptFooter vatable={data.cart.isNonVat }/>
        </Scrollbar>
        <Divider sx={{ my: 2 }} />
