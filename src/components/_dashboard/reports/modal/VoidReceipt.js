...
export default function VoidReceipt({ open, setOpen, fullData }) {
  const { data } = fullData;
  ...
  useEffect(() => {
    const computeVats = () => {
      ...
      if (
        data.cart.discounts.filter((x) => x.prefix === 'VAT' || x.prefix === 'SCD-5%').length > 0
      ) {
  ...
  const handlePrintAgain = async () => {
    ...
    try {
      await axios.post(`${Endpoints.TRANSACTION}/void/receipt`, formData, {
  ...
  const checkIfCustomerInfoNeed = (isVatZR, vatZrRepresentative, order) => {
    ...
    if (isVatZR) {
      ...
      customerInfo = `Customer: ${notGuest ? order.lastname.toUpperCase() + ', ' + order.firstname.toUpperCase() : ''
        }`;
    }
  ...
  return (
    <StyledModal open={open} BackdropComponent={Backdrop}>
      <ModalCard>
        ...
        <ReceiptHeader title="VOID" vatable={data.cart.isNonVat} />
          ...
          {data.cart.confirmOrders.map((order, index) => (
            <Box key={index} sx={{ borderBottom: '2px dashed #000' }}>
              <Box component="section" my={2}>
                ...
                <Typography noWrap variant="subtitle2">
                  {`Txn No.: ${fullData.txnNumber}`}
                </Typography>{' '}
                <Typography noWrap variant="subtitle2">
                  {`Void No.: ${data.cart.voidNumber}`}
                </Typography>
                <Typography noWrap variant="subtitle2">
                  {`Order ID.: ${data.cart.confirmOrders[0].orderId}`}
                </Typography>
                <Typography noWrap variant="subtitle2">
                  {`Date time: ${moment(data.cart.cartDate).format('MM-DD-YYYY hh:mm A')}`}
                </Typography>
              </Box>
              <Box
                component="section"
                      ...
                      specs.discounts.map((discount, index) => (
                        <Stack key={index} direction="row" justifyContent="space-between">
                          <Typography noWrap variant="subtitle2" ml={3}>
                            {`LESS ${discount.receiptLabel} ${discount.prefix === 'PERCENTAGE'
              ...
              <Typography noWrap variant="subtitle2" ml={3} my={1}>
                {`No. of Items: ${sum(order.products, 'quantity')}`}
              </Typography>
            </Box>
          ))}
          <Box sx={{ my: 1, borderBottom: '2px dashed #000' }}>
            ...
            {data.cart.payments.map((payment, index) => (
              <Box key={index}>
                ...
                {(payment.value === 'eWallet' || payment.value === 'cashOnDelivery') && (
                  <>
                    <Stack direction="row" justifyContent="space-between">
                      <Typography noWrap variant="subtitle2" ml={3}>
                        {payment.label}
                ...
                {payment.value.startsWith('CUSTOM::') && (
                  <>
                    <Stack direction="row" justifyContent="space-between">
                      <Typography noWrap variant="subtitle2" ml={3}>
                        {payment.label}
            ...
            <Typography noWrap variant="subtitle2" mt={1} mb={1}>
              {`VOID Remarks: ${data.cart.remarks}`}
            </Typography>
          </Box>
          ...
          {data.cart.discounts.filter((x) => x.prefix === 'LOYALTYPOINTS').length > 0 && (
            <Box component="section" sx={{ mt: 2 }}>
              <Typography noWrap variant="subtitle2">
                {`Customer Loyalty No.: ${data.cart.discounts.filter((x) => x.prefix === 'LOYALTYPOINTS')[0]
                  .customerLoyaltyId
                  }`}
              </Typography>
              <Typography noWrap variant="subtitle2">
                {`Previous Points: ${data.cart.discounts.filter((x) => x.prefix === 'LOYALTYPOINTS')[0].previousPoints
                  }`}
              </Typography>
              <Typography noWrap variant="subtitle2">
                {`Redeemed Points: ${data.cart.discounts.filter((x) => x.prefix === 'LOYALTYPOINTS')[0].redeemedPoints
                  }`}
              </Typography>
              <Typography noWrap variant="subtitle2">
                {`Remaining Points: ${data.cart.discounts.filter((x) => x.prefix === 'LOYALTYPOINTS')[0]
                  .previousPoints -
