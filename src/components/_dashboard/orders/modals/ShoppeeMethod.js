...
ShoppeeMethod.propTypes = {
  setOpen: PropTypes.func.isRequired
};

// ----------------------------------------------------------------------

export default function ShoppeeMethod({ setOpen }) {
  const state = store.getState();
  const { cart } = state;

  const [cash, setCash] = useState(cart.amounts.amountDue);
  ...
  const handleAddPayment = () => {
    ...
    store.dispatch(addPayment('shoppee', paymentData));
